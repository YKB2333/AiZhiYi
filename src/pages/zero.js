...
class Zero extends Component{
  ...
  */
    ...
    async componentWillMount(){
      ...
      let {data}= await this.props.axios.get('/fuli')
      this.setState({
        datalist:data
      })
    ...
    render(){
        ...
        return <div className="zero">
            <img src={require("../img/zero.jpg")} style={{width:"7.5rem",height:"3.5rem"}} />
            <ul>{
                this.state.datalist.map(item =><li onClick={this.toGoods.bind(this,item._id)} style={liStyle} key={item._id}>
                    ...
                    <div className="neirong" style={zeroCStyle}>
                        ...
                        <p style={p1Style}>邀请<span>{item.freeInviteNum}</span>好友，立即派送</p>
                        <p style={p2Style}><span>{item.goodsFicSalenum}</span>人已领</p>
                        <p style={p3Style}>￥<span>{item.freePrice}</span></p>
                        <p style={p4Style}>￥<span>{item.gPrice}</span></p>
                        <p style={
                            item.haveQualification?p5Style1:p5Style
                        }>{
                            item.haveQualification?"正在进行":"活动结束"
                        }</p>
